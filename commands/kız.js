    ...
    async run(client, message, args){
        ...
        if(qcount === 2){
            ...
            try{
                ...
                q1.roles.add(global.config.server.roles.girl.one)
                if(message.guild.roles.cache.get(global.config.server.roles.girl.two)) q1.roles.add(global.config.server.roles.girl.two)
                if(message.guild.roles.cache.get(global.config.server.roles.girl.three)) q1.roles.add(global.config.server.roles.girl.three)
                ...
                if(global.config.server.systems.logmsend === "evet") message.guild.channels.cache.get(global.config.server.channels.log).send({ embeds: [
                    new MessageEmbed()
                    .setDescription(`**${q1}, ${message.member} tarafından kayıt edildi! ( Kız )**`)
                ] })
                db.add(`user.${message.member.id}.reg.girl`, 1)
